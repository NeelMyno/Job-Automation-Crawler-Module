...
- **`pipeline/tracker.html`**: this repo doesn't ship one (application-tracking is out of scope
  here), so this half of `--write` quietly no-ops. If you want a persistent, deduped record across
  runs instead of a table that's rewritten fresh each time, add your own `tracker.html` with an
  `const APPLICATIONS = [` array in a `<script>` block and `crawl.py` will find and append to it.
  Otherwise a fresh `job-feed.md` each run is all you get, and the same posting can resurface on a
  later run with a wider `--hours` window.

Without `--write`, `crawl.py` prints its matches as JSON to stdout and touches nothing else. That's
